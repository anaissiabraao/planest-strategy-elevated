...
export default function Categories() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <h1 className="text-2xl md:text-3xl font-heading font-bold">Categorias</h1>
      <div className="flex gap-2">
        <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nova categoria" onKeyDown={(e) => e.key === "Enter" && add()} />
      ...
      <Card>
        <ul className="divide-y">
          ...
          {items.length === 0 && <li className="p-6 text-center text-sm text-muted-foreground">Sem categorias.</li>}
        </ul>
      </Card>
    </div>
  );
